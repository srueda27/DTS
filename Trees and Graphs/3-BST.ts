...
function rangeSumBST(root: BinaryTree | null, low: number, high: number): number {
  if (!root) return 0
...
function getMinimumDifference(root: BinaryTree | null): number {
...
function isValidBST(root: BinaryTree | null): boolean {
  ...
  const helper = (node: BinaryTree, left: number, right: number): boolean => {
    if (!node) return true
...
function closestValue(root: BinaryTree | null, target: number): number {
  ...
  if(target < root.val) val = closestValue(root.left, target)
  if(target > root.val) val = closestValue(root.right, target)
  
  if(val == -1) return root.val // Arbol de un sólo nodo retorna el mismo nodo
  
  if (Math.abs(root.val - target) == Math.abs(val - target)) return Math.min(root.val, val) // Si la distancia entre uno y el otro es la misma retorne el menor valor
   
  if(Math.abs(root.val - target) < Math.abs(val - target)) return root.val // Si la distancia es menor con el nodo, retorne el nodo
