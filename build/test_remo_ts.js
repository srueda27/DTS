function positionGuests(code, guests) {
    //check for a modify access code
    if (code.startsWith("delete-")) {
        const personToDelete = code.split("-")[1];
        let orderOnList = guests.findIndex(guest => guest[0] === personToDelete);
        if (orderOnList != -1) {
    ...
    let normalTable = [];
    let normalVipTable = [];
    let exclusiveVipTable = [];
    let superExclusiveVipTable = [];
    ...
    const namesOnly = listsOfTablePartners.map(partners => partners[1]); //Cleans the tuple to get only the names
