...
function createMeetingRoomingSystem() {
     ...
     */
    function isRoomAvailable(roomId: string, start: Date, end: Date): boolean {
     ...
     */
    function cancelBooking(roomId: string, start: Date, end: Date): boolean {
...
try {
    ...
    console.error(
        `Error occurred while booking the rooom for the meeting: ${error.message}`
    );
}

try {
    meetingRoom.cancelBooking(
        "",
        new Date("2024-05-20T10:00:00"),
